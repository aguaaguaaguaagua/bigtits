...
void msImageProcessor::GetModeCounts(vector<countsList> & clist)
{
	
	if(modePointCounts==NULL)
		return ;
	for(int i=0;i<regionCount;i++)
	{
		countsList tempElem(i,modePointCounts[i]);
		clist.push_back(tempElem);
	}
...
void msImageProcessor::NewOptimizedFilter2(float sigmaS, float sigmaR)
{
   ...
   if (N==3)
   {
      for(i=0; i<L; i++)
      {
         sdata[idxs++] = (i%width)/sigmaS;
         sdata[idxs++] = (i/width)/sigmaS;
         sdata[idxs++] = data[idxd++]/sigmaR;//R
         sdata[idxs++] = data[idxd++]/sigmaR;//G
         sdata[idxs++] = data[idxd++]/sigmaR;//B
      }
   } else if (N==1)
   {
      for(i=0; i<L; i++)
      {
         sdata[idxs++] = (i%width)/sigmaS;
         sdata[idxs++] = (i/width)/sigmaS;
         sdata[idxs++] = data[idxd++]/sigmaR;//gray
      }
   } else
   {
      for(i=0; i<L; i++)
...
//Test
	//for(i =0; i<height;i++)
	//{
	//	memset(modeTable+i*(width+1),1,width-i);
	//}
	for(i = 0; i < L; i++)
	{
		// if a mode was already assigned to this data point
